...
Page({
  ...
  onLoad: function (options) {
    ...
    that.getDishList(openid, merchant_id,1);      //获取酒楼菜品列表
    that.getMerchantInfo(openid, merchant_id);    //获取商家信息
    that.getMallGoodsList(openid,merchant_id,1);  //获取售全国商品
  },
  //获取酒楼菜品列表
  getDishList: function (openid, merchant_id,page){
    var that = this;
  ...
  //获取售全国商品
  getMallGoodsList:function(openid,merchant_id,page){
  ...
  loadMore: function (e) {
    ...
    if (tab =='take-out'){//外卖菜品
      var l_page = page;
      l_page += 1;
      that.getDishList(openid,merchant_id,l_page);
    }else {//售全国商品
      var l_page = page_mall;
      l_page += 1;
      that.getMallGoodsList(openid,merchant_id,l_page);
    }
  },
  /**
   * 菜品置顶
      ...
      success: function (res) {
        if (res.confirm) {
          utils.PostRequest(api_v_url + '/dish/putaway', {
            ...
            if (tab =='take-out'){
              that.getDishList(openid, merchant_id, page)
            }else {
              that.getMallGoodsList(openid, merchant_id, page_mall);
            }
  ...
  drawSharePicture: function (data) {
    ...
    wx.getImageInfo({
      src: data.object.picture,
      success: function (picture) {
        wx.getImageInfo({
          src: data.object.activePicture,
          success: function (activePicture) {
            wx.getImageInfo({
              ...
              success: function (qrCode) {
                ...
                let __x = 0,
                  ...
                  __pictureWidth = 0,
                  __pictureHeight = 0,
                  __pictureWidthRatio = 1,
                  __pictureHeightRatio = 1;
                ...
                x = 15;
                y = 15;
                let objectPictureWidth = 470,
                  objectPictureHeight = 300;
                ...
                let activePictureWidth = 125,
                  activePictureHeight = 125;
                x = fullWidth - activePictureWidth;
                y = 0;
                ...
                let tipBannerWidth = fullWidth,
                  tipBannerHeight = 50;
                x = 0;
                y = fullHeight - tipBannerHeight;
                ...
                fontSize = 24;
                x = fullWidth / 2 - tipContentMetrics.width * pixelRatio / 2;
                y = fullHeight - tipBannerHeight / 2 + fontSize / 2 - 2;
